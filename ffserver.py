...
def update_file(fileId):
    ...
    for key, value in data.items():
        if key in file_entry and value is not None:
            file_entry[key] = value

    current_time = datetime.now().isoformat()
    file_entry['updateAt'] = current_time
            
    fileTable.update({key: value}, doc_ids=[file_entry.doc_id])
...
def update_project(projectId):
    ...
    current_time = datetime.now().isoformat()
    project_entry['updateAt'] = current_time
...
@FFapp.route('/deleteCollection/<string:projectId>', methods=['DELETE'])
def delete_collection(projectId):
    # 컬렉션 삭제 요청
    response = requests.delete(f"http://localhost:6333/collections/{projectId}")
    
    if response.status_code == 200:
        return jsonify({'success': True, 'message': 'success to delete'}), 200
    else:
